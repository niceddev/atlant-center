...
@else
    <div class="overflow-x-auto relative shadow-md sm:rounded-lg">
        ...
        <ul id="servicesTab" class="flex flex-wrap -mb-px text-sm font-medium text-center" data-tabs-toggle="#servicesTabContent" role="tablist">
        ...
        <div id="servicesTabContent" id="{{ $key }}" >
            @foreach(config('app.languages') as $key => $lang)
                    ...
                    <tbody>
                        @foreach($services as $service)
                            <tr class="max-h-7 grid-cols-4 bg-white border-b hover:bg-gray-50">
                                ...
                                <td class="py-4 px-6">
                                    {{ $service->getTranslation('title', $key) }}
                                </td>
                                <td class="py-4 px-6">
                                    {!!
                                        (strlen($service->getTranslation('content', $key)) > 50)
                                        ? substr($service->getTranslation('content', $key), 0, 50) . '...'
                                        : $service->getTranslation('content', $key)
                                    !!}
                                </td>
                                <td class="py-4 px-6">
